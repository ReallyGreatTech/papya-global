...
async def process_face_fusion(
    ...
    try:
        ...
        # Construct command for the first run (commented out)
        first_command = [
            sys.executable,  # Use current Python interpreter
            "facefusion.py",
            "headless-run",
        ...
        # Log the first command
        first_command_str = " ".join(first_command)
        logger.info(f"Mocking first command: {first_command_str}")

        # Mock the first command execution
        returncode, stdout, stderr = 0, "Mocked first command execution", ""

        # Generate unique output filename for the second run
        fname = os.path.splitext(sfname)[0].replace('.', '').replace(' ', '_') if sfname else "final"
        second_output_path = os.path.join(OUTPUT_DIR, f"output_{job_id}_{fname}.mp4")

        # Construct command for the second run (commented out)
        second_command = [
            sys.executable,  # Use current Python interpreter
            "facefusion.py",
            "headless-run",
        ...
        # Log the second command
        second_command_str = " ".join(second_command)
        logger.info(f"Mocking second command: {second_command_str}")

        # Mock the second command execution
        returncode, stdout, stderr = 0, "Mocked second command execution", ""

        end_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # Simulate successful completion
        logger.info(f"Job {job_id} completed successfully (mocked)")
        job_statuses[job_id] = JobStatus(
            job_id=job_id,
            status="completed",
            output_path=second_output_path,
        ...
        # Upload the output to S3 and get the URL (mocked)
        url = "https://example.com/mocked_video_url.mp4"
        ...
        # Send email to admin or user based on flag (keep commented for future use)
        if flag:
            await service_module.send_email(None,  url, admin_email, "Admin")
        else:
            await service_module.send_email(None,  url, email, "User")
        ...
        # Send error email
        await service_module.send_email(None, error_msg, email, "User")
...
async def create_face_fusion_job(
    ...
    try:
        admin_email = "sredeemer24@gmail.com"
